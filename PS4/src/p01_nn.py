...
def backward_softmax(x, grad_outputs):
    ...
    dydx = -y[:, np.newaxis] @ y[:,np.newaxis].T
    np.fill_diagonal(dydx, y*(1-y))
    return dydx @ grad_outputs
    # *** END CODE HERE ***
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    input_channels, input_width, input_height = data.shape
    dw, dx, db = np.zeros_like(conv_W), np.zeros_like(data), np.zeros_like(conv_b)

    for x in range(input_width - conv_width + 1):
        for y in range(input_height - conv_height + 1):
            for c in range(conv_channels):
                dw[c, :, : ,:] += data[:, x:(x+conv_width), y:(y+conv_height)] * output_grad[c, x, y]
                dx[:, x:(x+conv_width), y:(y+conv_height)] += conv_W[c, :, :, :] * output_grad[c, x, y]
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    input_channels, input_width, input_height = data.shape
    out_channels, out_width, out_heigth = output_grad.shape
    ...
    for c in range(out_channels):
        for x in range(0, input_width, pool_width):
            for y in range(0, input_height, pool_height):
                ...
                dout[c, x+di, y+dj] = output_grad[c, x//pool_width, y//pool_height]
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    dw = data[:, np.newaxis] @ output_grad[:, np.newaxis].T
    dx = weights @ output_grad
    db = output_grad
    return dw, db, dx
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params):
    ...
    ds = backward_softmax(logits, dy)
    dW2, db2, dx = backward_linear(W2, b2, flattened, ds)
    dx = backward_relu(first_max_pool, np.reshape(dx, first_after_relu.shape))
    dx = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, dx)
    dW1, db1, dx = backward_convolution(W1, b1, data, dx)

    return {'W1': dW1, 'W2': dW2, 'b1': b1, 'b2': b2}
    # *** END CODE HERE ***
